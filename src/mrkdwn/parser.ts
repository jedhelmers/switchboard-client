...
function parseBlockquote(
  ...
  while (i < lines.length && isBlockquoteLine(lines[i])) {
    if (lines[i] === '>') innerLines.push('')
    else innerLines.push(lines[i].slice(2))
    i++
  }
...
function parseList(
  ...
  const items: MrkdwnNode[][] = []
  let i = start
  while (i < lines.length) {
    const marker = listMarker(lines[i])
    if (marker === '') break
    if ((marker === '-') === ordered) break // marker kind switched
...
function emphasisStyle(ch: string): MrkdwnStyle {
  if (ch === '*') return 'bold'
  if (ch === '_') return 'italic'
  return 'strike'
}
...
function canOpenEmphasis(text: string, i: number): boolean {
  if (!atWordBoundary(text, i)) return false
  if (i + 1 >= text.length) return false
  const next = text[i + 1]
  if (isSpaceChar(next)) return false
  if (next === text[i]) return false // rule 9: repeated marker
  if (i > 0 && text[i - 1] === text[i]) return false // rule 9, left side
  return true
}
...
function findCodeClose(text: string, start: number): number {
  for (let j = start; j < text.length; j++) {
    if (text[j] === '\n') return 0
    if (text[j] === '`') return j
  }
...
function findLinkClose(text: string, start: number): number {
  for (let j = start; j < text.length; j++) {
    if (text[j] === '\n') return 0
    if (text[j] === '>') return j
  }
...
function splitLink(inner: string): { url: string; label: string } {
  const idx = inner.indexOf('|')
  if (idx >= 0) return { url: inner.slice(0, idx), label: inner.slice(idx + 1) }
...
function matchBareURL(
  text: string,
  i: number,
): { url: string; advance: number } | null {
  const lower = text.slice(i).toLowerCase()
  let prefixLen = 0
  if (lower.startsWith('https://')) prefixLen = 8
  else if (lower.startsWith('http://')) prefixLen = 7
  else return null

  let end = i + prefixLen
  while (end < text.length && !isSpaceChar(text[end])) end++
  if (end === i + prefixLen) return null // scheme only
  ...
  if (url.length <= prefixLen) return null
  return { url, advance: url.length }
}

// -------- plaintext --------

function blockToPlain(n: MrkdwnNode): string {
  switch (n.type) {
    case 'paragraph':
...
function inlineToPlain(ns: MrkdwnNode[]): string {
  let s = ''
  for (const n of ns) {
    switch (n.type) {
      case 'text':
